...
namespace Tamaris.API.Controllers.Admin
{
	...
	public class RolesController : BaseController
	{
		...
		/// <summary>
		/// Gets paging list with all users (ForSelect) for given role
		/// </summary>
		/// <remarks>With pagination, you can optimize fetching the users. For example, you can fetch 100 pages with 30 pages per page or 30 pages with 100 users per page.
		/// Additionally, if you opt to use the pagination (you provide the values for pageIndex and pageSize), this method will also include X-Pagination header
		/// with additional information about the result like totalPages, currentPage, next-/previous link, has next-/previous link.
		/// </remarks>
		/// <param name="id">The key of the role we want to fetch the users for</param>
		/// <param name="parameters">Query parameters</param>
		...
		/// <response code="401">If the user is not authorized to access this resource</response>
		[HttpGet("{id}/AdminUsersForSelectForRoles", Name = "GetAdminUsersForSelectForRoles")]
		[ProducesResponseType(StatusCodes.Status200OK, Type = typeof(UserForSelect))]
		[ProducesResponseType(StatusCodes.Status204NoContent)]
		[ProducesResponseType(StatusCodes.Status401Unauthorized)]
		public async Task<ActionResult<IEnumerable<UserForSelect>>> GetAdminUsersForSelectForRoles(int id, [FromQuery] QueryParameters parameters, string searchString, CancellationToken cancellationToken = default)
		{
			LogMethodListGetterEntry(parameters, searchString);

			try
			{
				var users = await _unitOfWork.UsersRepository.GetPaginatedForSelect_ForRolesAsync(id, parameters, searchString, cancellationToken);
				...
				return NoContent(); // No users (ForSelect) for the given role and index/page combination!
			}
			catch(TaskCanceledException)
			{
				LogVerbose("User cancelled action.");
